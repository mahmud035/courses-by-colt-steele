...
/* 
{
  ...
  // Lexical Environment: The JavaScript engine creates a mapping of variables at the time the code is parsed (aka read), determining what variables a function will have access to.
...
/* 
{
  ...
  const createCounter = () => {
    ...
    return {
      increment: () => ++count, // The closure captures and updates `count`
      decrement: () => --count,
      getCount: () => count, // The closure can access the current `count`
    };
  };
...

  {
    ...
    const createEmployee = (name, salary) => {
      return {
        getName: () => name,
        getSalary: () => salary,
        increaseSalary: (amount) => (salary += amount),
      };
    };
